...
handler.is_wrapped True while (config' in prompt:
...
handler.send command('exit', expect string-r' [^\n]\S+#\WS")

prompt handler.find_prompt()

if is config mode:

#we have reached the vtysh root using exit command(s) return

#if we are in vtysh root, do the following

handler.send_command('configure terminal', expect string-r'[^\n]\S+#\WS") assert (config' in handler.find_prompt(), 'Failed to enter config mode from vtysh
